...
# These are annotations defined in the Checker Framework that should go on
# their own line.  Their @Target annotation contains neither "TYPE_USE" nor
# "{}".  To generate this list (last done on 2016-06-23):
#   ag --file-search-regex src/org/checkerframework --files-with-matches '^@Target\b(?!.*TYPE_USE)(?!.{})' $cf | sed 's/.*\///' | sed 's/\(.*\)\.java/    "\1",/' | sort
